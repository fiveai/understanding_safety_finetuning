...
for layer_num in range(num_layers):
    ...
    lst_safe_row_space.append(lst_index_safe)
    lst_unsafe_row_space.append(lst_index)
...
lst_safe = np.abs(np.swapaxes(np.array(lst_safe_row_space), -1,-2))
print(lst_safe.shape)

lst_unsafe = np.abs(np.swapaxes(np.array(lst_unsafe_row_space), -1,-2))
print(lst_unsafe.shape)
...
for i in range(len(lst_safe)):
    lst_mean_safe_dup = []
    lst_mean_unsafe_dup = []
    lst_std_safe_dup = []
    lst_std_unsafe_dup = []
    for j in range(len(lst_safe[i])):
        lst_mean_safe = []
        lst_mean_unsafe = []
        lst_std_safe = []
        lst_std_unsafe = []
        for k in range(len(lst_safe[i][j])):
            mean_safe = np.mean(np.array(lst_safe[i][j][k]))
            mean_unsafe = np.mean(np.array(lst_unsafe[i][j][k]))
            lst_mean_safe.append(mean_safe)
            lst_mean_unsafe.append(mean_unsafe)
            std_safe = np.std(np.array(lst_safe[i][j]))
            std_unsafe = np.std(np.array(lst_unsafe[i][j]))
            lst_std_safe.append(std_safe)
            lst_std_unsafe.append(std_unsafe)
        lst_mean_safe_dup.append(lst_mean_safe)
        lst_mean_unsafe_dup.append(lst_mean_unsafe)
        lst_std_safe_dup.append(lst_std_safe)
        lst_std_unsafe_dup.append(lst_std_unsafe)
        
    arr_mean_safe.append(lst_mean_safe_dup)
    arr_mean_unsafe.append(lst_mean_unsafe_dup)
    arr_std_safe.append(lst_std_safe_dup)
    arr_std_unsafe.append(lst_std_unsafe_dup)

arr_mean_safe = np.abs(np.array(arr_mean_safe))
arr_mean_unsafe = np.abs(np.array(arr_mean_unsafe))
arr_std_safe = np.abs(np.array(arr_std_safe))
arr_std_unsafe = np.abs(np.array(arr_std_unsafe))
...
layer_num = [1,2,3,4,5,6]
for i in range(6):
 for j in range(1):
  if j==0:
    axs[i].scatter(np.arange(np.array(arr_mean_safe[i][-1][:counter_values]).shape[0]),np.array(arr_mean_safe[i][-1][:counter_values]), color='green', marker=marker_lst[0],s=15, label='Usf - Sf Clt, Sf Smp')
    axs[i].scatter(np.arange(np.array(arr_mean_unsafe[i][-1][:counter_values]).shape[0]),np.array(arr_mean_unsafe[i][-1][:counter_values]), color='red', marker=marker_lst[1],s=15, label='Usf - Sf Clt, Usf Smp')
    axs[i].fill_between(np.arange(np.array(arr_std_safe[i][-1][:counter_values]).shape[0]), np.array(arr_mean_safe[i][-1][:counter_values])-np.array(arr_std_safe[i][-1][:counter_values]), np.array(arr_mean_safe[i][-1][:counter_values])+np.array(arr_std_safe[i][-1][:counter_values]),alpha=0.5,color=colour_lst[0])
    axs[i].fill_between(np.arange(np.array(arr_std_unsafe[i][-1][:counter_values]).shape[0]), np.array(arr_mean_unsafe[i][-1][:counter_values])-np.array(arr_std_unsafe[i][-1][:counter_values]), np.array(arr_mean_unsafe[i][-1][:counter_values])+np.array(arr_std_unsafe[i][-1][:counter_values]),alpha=0.5,color=colour_lst[1])
    axs[i].set_xlabel("Layer Number: {} ".format(layer_num[i]), fontsize=20)
    axs[i].set_ylim(0,3)
  axs[i].set_xticks((0,4,9,14), (1,5,10,15))
...
plt.tight_layout()
plt.savefig("./plots_row_space/" + args.plot_path + '/' + args.data_test + ".pdf")
plt.close('all')