...
print("harmful_arr shape", harmful_arr.shape)
for i in range(harmful_arr.shape[0]):
        for j in range(harmful_arr.shape[1]):
            harmful_arr = harmful_cache['pre_linear', layer][:, pos, :].detach().cpu().float().numpy()
            harmless_arr = harmless_cache['pre_linear', layer][:, pos, :].detach().cpu().float().numpy()
            np.matmul(np.expand_dims(harmful_arr[layer][i],axis=1),np.expand_dims(lst_unsafe[layer][i],axis=0))
for layer in range(24):
    
        # left_sv_unsafe, right_sv_unsafe, sing_values_unsafe = get_svd(harmful_arr[i])
        # left_sv_safe, right_sv_safe, sing_values_safe = get_svd(harmless_arr[i])
...
for layer in range(24):
    ...
    for i in range(args.num_eigenvalues):
        sub=sub+np.matmul(np.expand_dims(lst_unsafe[layer][i],axis=1),np.expand_dims(lst_unsafe[layer][i],axis=0)) - np.matmul(np.expand_dims(lst_safe[layer][i],axis=1),np.expand_dims(lst_safe[layer][i],axis=0))
    sub_lst.append(sub)
...
sd = model.state_dict()
for w_val in [1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]:
