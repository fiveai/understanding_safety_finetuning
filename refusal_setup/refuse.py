...
for layer in range(24):
    left_sv_unsafe, right_sv_unsafe, sing_values_unsafe = get_svd(harmful_cache['pre_linear', layer][:, pos, :].detach().cpu().float().numpy())
    left_sv_safe, right_sv_safe, sing_values_safe = get_svd(harmless_cache['pre_linear', layer][:, pos, :].detach().cpu().float().numpy())
